...
echo "Installing Golang..." | tee -a "$LOG_FILE"
apt install -y golang &>> "$LOG_FILE"

if go version &>> "$LOG_FILE"; then
    echo "Golang installed successfully." | tee -a "$LOG_FILE"
else
    echo "Golang installation failed. Check the log file for details." | tee -a "$LOG_FILE"
    exit 1
fi
...
echo "Golang version: $(go version)" | tee -a "$LOG_FILE"
